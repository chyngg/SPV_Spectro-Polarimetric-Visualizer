...
with dpg.group(parent='left_menu', indent=5) as left_menu:
	...
	with dpg.group(tag="history_group"):
		with dpg.child_window(tag="checkbox_area", height=400, parent="history_group", autosize_x=True, border=True):
			pass
...
with dpg.group(parent='tools', indent=5) as tools:
	with dpg.tab_bar(tag="tools_tab_bar"):
		with dpg.tab(label="SP_image", tag="tools_tab_sp"):
			...
			dpg.add_separator()
			dpg.add_button(label="Distribution of Stokes vector", callback=lambda: histogram.show_stokes_histogram(1),
						   width=-1)
			dpg.add_spacer(height=5)
			dpg.add_separator()
...
with dpg.group(parent='center_img', indent=5) as center_img:
	with dpg.texture_registry(show=False):
		dpg.add_dynamic_texture(
			CANVAS_WIDTH,
			CANVAS_HEIGHT,
			np.zeros((CANVAS_HEIGHT, CANVAS_WIDTH, 4), dtype=np.float32).flatten(),
			tag="uploaded_texture",
		)
		dpg.add_dynamic_texture(
			CANVAS_WIDTH,
			CANVAS_HEIGHT,
			np.zeros((CANVAS_HEIGHT, CANVAS_WIDTH, 4), dtype=np.float32).flatten(),
			tag="histogram_texture",
		)

	# right viewer (for Histogram)
	with dpg.group(tag="center_split_container"):
